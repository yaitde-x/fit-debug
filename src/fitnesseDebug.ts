...
export class FitnesseDebugSession extends LoggingDebugSession {

	private static threadID = 1;

	// a Mock runtime (or debugger)
	private _runtime: FitnesseRuntimeProxy;
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', FitnesseDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', FitnesseDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', FitnesseDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', FitnesseDebugSession.threadID));
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(FitnesseDebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
